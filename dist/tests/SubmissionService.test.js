...
describe('SubmissionService Integration Tests', () => {
    ...
    beforeEach(async () => {
        ...
        mockConnection = setup_1.mockSolanaConnection;
        submissionService = new SubmissionService_1.SubmissionService(mockConnection, dbService);
        // Mock PublicKey validation
        web3_js_1.PublicKey.isOnCurve = jest.fn().mockReturnValue(true);
    });
    afterEach(async () => {
    ...
    describe('submitToken', () => {
        ...
        it('should successfully submit a new valid token', async () => {
            ...
            const submissions = await dbService.query('SELECT * FROM token_submissions WHERE token_address = ?', [validSubmission.tokenAddress]);
